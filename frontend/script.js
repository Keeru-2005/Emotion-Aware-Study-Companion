...
window.addEventListener("blur", () => { metrics.active_tab = false; });
window.addEventListener("focus", () => { metrics.active_tab = true; });
window.addEventListener("mousemove", () => { metrics.last_mouse_move = Date.now(); });
window.addEventListener("keydown", () => { metrics.last_keystroke = Date.now(); });
...
async function triggerPauseQuiz() {
  ...
  try {
    ...
    document.getElementById("poLoading").innerHTML = `<div style="color:#d32f2f;text-align:center;">
        <b>⚠️ AI Server Error</b><br/>Failed to generate question.<br/>(Check API limits or reload)
        <br/><button onclick="closePauseOverlay()" style="margin-top:10px;padding:5px 10px;border-radius:5px;border:none;background:#ddd;cursor:pointer;">Dismiss</button>
      </div>`;
    document.querySelector(".po-spinner").style.display = "none";
    // Do NOT auto-close or resume video! Let the user read the error.
  }
}
...
`.trim();
  ...
  const data = await res.json();
  // Strip out potential markdown wrapping or text before parsing JSON
...
async function pollEmotion() {
  ...
  try {
    ...
    else if (emotion === "fear")                         { state = "Confused";     attention = "Low";    }
    else if (emotion === "distracted")                   { state = "Distracted";   attention = "Zero";   }
    else if (emotion === "idle")                         { state = "Idle";         attention = "Zero";   }
...
async function handleEmotionAction(emotion) {
  ...
  if (emotion === "distracted") {
    ...
    showActionCard("stressed", "⚠️ Distracted", "Please focus on the screen!");
    return;
  } else if (emotion === "idle") {
    showActionCard("bored", "💤 Idle", "Are you still there?");
    return;
  }
...
function toggleDiagramDrawer() {
  const el = document.getElementById('diagramDrawer');
...
async function sendMessage() {
  ...
  try {
    ...
    const wantsDiagram = question.toLowerCase().match(/(diagram|flowchart|map)/);
    if (wantsDiagram) {
      finalPrompt += `

IMPORTANT: You MUST include a Mermaid.js diagram.
- Start exactly with \`graph TD\`
- Only use standard arrows \`-->\`
- DO NOT use any brackets '[]' or parentheses '()' or special characters in node text.
- Wrap exactly in \`\`\`mermaid and \`\`\` tags.`;
    }
    ...
    if (wantsDiagram && answerText.includes("```mermaid")) {
       const parts = answerText.split("```mermaid");
       const chartPart = parts[1].split("```")[0].trim();
       answerText = parts[0] + (parts[1].split("```")[1] || "");
       
       const canvas = document.getElementById("diagramCanvas");
       if(canvas) {
           canvas.innerHTML = `<pre class="mermaid">${chartPart}</pre>`;
           try {
               mermaid.run({ nodes: [canvas.querySelector('.mermaid')] });
               document.getElementById("toggleDiagramBtn").classList.add("active");
               toggleDiagramDrawer(); // Open drawer auto
           } catch (e) {
               console.log("Mermaid syntax error:", e);
           }
       }
    }

    thinkingEl.innerHTML = escHtml(answerText).replace(/\n/g, '<br>');
